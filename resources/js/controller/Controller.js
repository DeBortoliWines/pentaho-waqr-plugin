...
Controller = function( wiz, mdModels, fileName )
{
  /*WaqrWizard*/
  this.wiz = wiz;
  ...
  // If there is an existing report, then load it, NOTE: runs synchronously
  if(this.fileName != null) {
	  this.reportSpecDoc = this.solutionRepository.getWaqrReportSpecDoc(this.fileName);
...
Controller.prototype.getModel = function() 
{
  ...
  var localThis = this;
  pentahoGetResource( CONTEXT_PATH, "api/repos/xwaqr/resources/xsl/businessviewlist.xsl", 
    function ( xmlStr )
    {
      localThis.viewsXsl = XmlUtil.load( xmlStr );
      pentahoGetResource( CONTEXT_PATH, "api/repos/xwaqr/resources/xsl/businessview.xsl", 
        function( xmlStr )
        {
...
Controller.prototype.enableReportSaveBtns = function()
{
  ...
  if(window.parent && window.parent.mantle_initialized == true){
      window.parent.enableSave( bEnable && !bIsSaving );
  }
  ...
  if ( rsStatus.status == Status.OK )
  {
    ...
    if (window.parent != null && window.parent.mantle_initialized == true) {
      // open preview tab for waqr
		  ...
		  window.parent.mantle_execute_url_in_new_tab(url, xml, 'Preview', 'Report Preview');
	} else {
 ...
 */
function pentahoGetResource( url, query, func, mimeType ) {
